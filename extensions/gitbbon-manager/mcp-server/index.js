 ...
 * 워크스페이스 루트의 .gitbbon/state/context.json 을 읽어 아래 3개 tool을 제공한다.
 *
...
function findContextFile() {
	...
	// cwd 부터 상위로 올라가며 .gitbbon/state/context.json 탐색
	let dir = process.cwd();
	for (let i = 0; i < 10; i++) {
		const candidate = path.join(dir, '.gitbbon', 'state', 'context.json');
	...
	return path.join(process.cwd(), '.gitbbon', 'state', 'context.json');
}
