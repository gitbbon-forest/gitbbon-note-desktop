---
name: test
description: gitbbon-chat Extension Host 테스트 실행. 실패 시 원인 분석 → 코드 수정 → 재실행 루프 수행. 사용법: /test electron gitbbon-chat
---

# /test — gitbbon-chat Extension 테스트 러너
...
다른 Extension 테스트 추가 시:
1. 해당 Extension에 `@vscode/test-electron` 설정 (PR #114 참고)
2. 이 SKILL.md의 레지스트리에 행 추가

## ARGUMENTS

- `electron gitbbon-chat` → gitbbon-chat Extension 테스트 실행
- 인자 없음 → `electron gitbbon-chat` 실행 (현재 유일한 구현)
